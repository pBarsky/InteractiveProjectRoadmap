...
describe('<Login />', () => {
	...
	it('should submit when form inputs contain text', async () => {
		...
		act(() => {
			...
			userEvent.click(getByRole('button', { name: defaultDict.forms.buttons.login.text }));
		});

		waitFor(() => expect(onSubmit).toHaveBeenCalledTimes(1));

		await waitFor(() => {
			expect(queryByText(customErrorMessages.email.required)).not.toBeInTheDocument();
			expect(queryByText(customErrorMessages.password.required)).not.toBeInTheDocument();
		});
	});
