...
export const roadmapFormValuesSchema: SchemaOf<RoadmapFormValues> = object()
  .shape({
    ...
    description: string().notRequired().max(2048, validationErrorMessages.description.max(2048)),
    startsOn: date().required(validationErrorMessages.startsOn.required),
    endsOn: date().notRequired().min(ref('startsOn'), validationErrorMessages.endsOn.failedTime)
  })
  .defined();
